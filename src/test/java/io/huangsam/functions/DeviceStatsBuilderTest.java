...
public class DeviceStatsBuilderTest {
    ...
    public void testDeviceStatsBuilding() {
        List<KV<String, Long>> kvPairs = Arrays.asList(
                KV.of("device-1", 10L),
                KV.of("device-2", 15L),
                KV.of("device-3", 8L)
        );

        PCollection<DeviceStats> stats = pipeline
                .apply(Create.of(kvPairs))
                .apply(Window.into(FixedWindows.of(Duration.standardSeconds(5))))
                .apply(ParDo.of(new DeviceStatsBuilder()))
        ...
        PAssert.that(stats)
                .satisfies((Iterable<DeviceStats> statsList) -> {
