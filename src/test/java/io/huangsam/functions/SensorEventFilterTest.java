...
public class SensorEventFilterTest {
	...
	public void testSensorEventFiltering() {
		List<DeviceEvent> events = Arrays.asList(new DeviceEvent("device-1", "SENSOR_READING_1"),
				new DeviceEvent("device-2", "SYSTEM_EVENT"), new DeviceEvent("device-3", "SENSOR_READING_2"),
				new DeviceEvent("device-4", "SENSOR_ERROR_5"));

		PCollection<DeviceEvent> sensorEvents = pipeline.apply(Create.of(events).withCoder(DeviceEventCoder.of()))
				.apply(Filter.by(new SensorEventFilter())).setCoder(DeviceEventCoder.of());

		PAssert.that(sensorEvents).containsInAnyOrder(new DeviceEvent("device-1", "SENSOR_READING_1"),
